...
13. [Joint OpenVid + Full Mobile-O 60k Benchmark Snapshot (2026-05-12)](#joint-openvid--full-mobile-o-60k-benchmark-snapshot-2026-05-12)
...
## Joint OpenVid + Full Mobile-O 60k Benchmark Snapshot (2026-05-12)

We now have a qualitative benchmark snapshot for the current joint
`OpenVid + full_mobile_o` line after a meaningful amount of training.

### Checkpoint under test

- Hugging Face export:
  `stage1_joint_openvid_fullmobile_o_fulldit_diffonly_initlatest_bs64_v2_20260429_8gpu_60k.pt`
- Actual stored step inside the checkpoint payload:
  `step=63600`
- Student family:
  `SmolVLM2-500M`
- Projector:
...
- DiT setting:
  `train_modules: [all]`
- Objective:
  diffusion-only joint training
- Hardware used for the run:
  `8 x H200`

### Training line being benchmarked

This checkpoint belongs to the joint image+video continuation line that mixes:

- `OpenVid` video data
- merged `JourneyDB + Short-Caption` image data

The active stable joint recipe used for this line is:

- config:
  `configs/stage1_joint_openvid_fullmobile_o_fulldit_diffonly_initlatest_v2_bs64_8gpu.yaml`
- launcher:
  `scripts/train_joint_openvid_fullmobile_o_fulldit_diffonly_initlatest_v2.sh`
- joint schedule:
  interval-based image/video alternation
- current micro-batch setting used for stable training:
  `video batch_size=1`, `image batch_size=4`, `grad_accum_steps=1`

### Inference recipe used for this benchmark

All qualitative tests below were run with the same inference recipe:

- env:
  `mobileov_onepass_20260401`
- backend:
  `fixed`
- dtype:
  `bf16`
...
- root:
  `output/hf_joint_60k_test`
- images/videos:
  `output/hf_joint_60k_test/generated`
- video frame triptychs:
  `output/hf_joint_60k_test/video_frame_samples`
...
- fox portrait in snowfall
- Japanese alley after rain
- minimalist still life with cup + orange

Video sanity prompts:

- golden retriever running along a beach at sunset
- astronaut riding a horse on the moon
- bustling rainy city street at night
- Japanese woman in a cherry blossom garden
- overhead group dancing near a pool
- overhead single adult woman dancing near a pool

### Main qualitative findings

Strengths:

- The checkpoint keeps a strong cinematic prior.
- Animal/action prompts remain a relative strength.
- The `golden retriever` video is one of the cleanest prompt-following cases.
- The `astronaut riding a horse on the moon` video is visually strong and
  conceptually faithful.
- On the image side, `mountain`, `fox`, and `Japanese alley` remain solid
  showcase prompts.

Weaknesses:

- Prompt following is uneven across domains.
- The `still life` image is weaker than the stronger cinematic prompts and does
  not anchor object composition as reliably.
- Multi-person choreography and explicit camera instructions remain weak.
- Both poolside dancing prompts drift toward a generic fashion/poolside visual
  manifold instead of obeying the requested `overhead` camera view and
  dance-specific motion.
- The single-subject poolside prompt behaves better than the multi-person
  version, but it still fails to truly honor the overhead-camera instruction.

### Interpretation

The current `60k` checkpoint does **not** look uniformly bad. It looks
specialized.

The best description at this stage is:

- strong aesthetics
- strong cinematic lighting and color
- reasonable motion on some action prompts
- uneven prompt fidelity
- weak structured camera control
- bias toward hero-shot / portrait / beauty-composition solutions

This behavior is consistent with an over-specialized or partially overfit
joint line rather than a broadly robust generalist checkpoint.

In practice:

- it can still produce attractive outputs for a curated subset of prompts
- but it should not yet be considered a stable benchmark-quality checkpoint
  for broad prompt coverage

### Practical takeaway

This `60k` joint checkpoint is worth preserving as an important reference
point, but not yet as the default “best overall” joint model.

It is currently best viewed as:

- a useful checkpoint for studying how joint training changes motion/aesthetic
  behavior,
- a checkpoint with clear strengths on cinematic prompts,
- and a checkpoint that still needs better prompt-control balance before it can
  serve as a reliable broad benchmark candidate.
